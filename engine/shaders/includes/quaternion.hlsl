...
// http://mathworld.wolfram.com/Quaternion.html
float3 quat_rotate_vector(float3 v, float4 r)
{
    float4 r_c = r * float4(-1, -1, -1, 1);
    return quat_mul(r, quat_mul(float4(v, 0), r_c)).xyz;
}

// A given angle of rotation about a given axis
float4 quat_rotate_angle_axis(float angle, float3 axis)
{
...
float4 quat_from_to_rotation(float3 v1, float3 v2)
{
    float4 q;
    float d = dot(v1, v2);
    if (d < -0.999999)
    {
        ...
        tmp = normalize(tmp);
        q = quat_rotate_angle_axis(PI, tmp);
    }
    else if (d > 0.999999) {
...
{
    float4 conj = quat_conj(q);
    return conj / (q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w);
}

float4 quat_diff(float4 q1, float4 q2)
{
    return q2 * quat_inverse(q1);
}

float4 quat_look_at(float3 forward, float3 up)
{
...
float4 quat_slerp(in float4 a, in float4 b, float t)
{
...
float4x4 quat_to_matrix(float4 quat)
{
