...
namespace core { namespace Device {
	...
	Texture::Texture(const TextureInitializer& initializer)
	{
		...
		{
			SetupRGBA(initializer);
		}
		else if (initializer.mode == TextureInitializer::Mode::DepthBuffer) // Depth texture
		{
			SetupDepth(initializer);
		}
		else
			throw std::runtime_error("unsupported mode");
	}

	void Texture::SetupRGBA(const TextureInitializer& initializer)
	{
		auto allocator = Engine::GetVulkanContext()->GetAllocator();
		auto& device = Engine::GetVulkanDevice();
	...
	void Texture::SetupDepth(const TextureInitializer& initializer)
	{
		auto allocator = Engine::GetVulkanContext()->GetAllocator();
		auto& device = Engine::GetVulkanDevice();
